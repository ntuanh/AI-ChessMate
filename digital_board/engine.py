"""Bọc Stockfish cho bàn cờ số, kèm phần dò binary.

**Vì sao không dùng lại `chess_ai.engine`**: `ChessEngine` ở đó đóng đinh
`config.STOCKFISH_PATH = "/usr/games/stockfish"` — đúng cho AIBOX, nhưng không
khởi động nổi trên Windows/macOS, nơi bàn cờ số cần chạy được ngay khi chưa có
phần cứng.  Nó cũng không cache và không có khái niệm ``loss`` (nước này kém nước
tốt nhất bao nhiêu centipawn), thứ mà giao diện gợi ý cần để xếp hạng phương án.
Hai module cùng gọi Stockfish nhưng phục vụ hai bài toán khác nhau.

Một tiến trình sống lâu, không phải mỗi truy vấn một tiến trình: ở 300 ms thì tạo
tiến trình còn tốn hơn cả tìm kiếm.  Phân tích được cache theo
(FEN, multipv, movetime) vì một phiên xem đi xem lại cùng một thế cờ liên tục —
bấm gợi ý hai lần, undo rồi đi lại, tải lại trang.

``find_binary`` không phụ thuộc hệ điều hành: nó dò đúng những chỗ binary thật sự
xuất hiện, theo thứ tự ý định rõ ràng dần, và mọi caller nhận cùng một đáp án.
"""
...
#: Chỗ trình quản lý gói thường đặt binary.
SYSTEM_PATHS = (
    "/usr/games/stockfish",
...
def _kit_path() -> Optional[str]:
    """`chess_ai.config.STOCKFISH_PATH` nếu import được.

    Trên AIBOX thì hai gói dùng chung một binary; nếu ai đó đổi đường dẫn trong
    config, bàn cờ số đi theo luôn thay vì lệch âm thầm.  Import trong hàm và
    nuốt lỗi vì `chess_ai` kéo theo numpy/cv2 — những thứ máy chạy bàn cờ số
    không bắt buộc phải có.
    """
    try:
        from chess_ai import config  # noqa: PLC0415 - cố ý import muộn
    except Exception:  # noqa: BLE001 - thiếu chess_ai là chuyện bình thường
        return None
    return getattr(config, "STOCKFISH_PATH", None)
...
def find_binary(explicit: Optional[str] = None) -> Optional[str]:
    """Tìm binary Stockfish, hoặc trả None.

    Thứ tự, ý định rõ ràng nhất trước:

    1. ``explicit`` — truyền qua dòng lệnh.
    2. ``$STOCKFISH_PATH`` (hoặc ``$STOCKFISH``).
    3. ``engine/`` trong repo, chỗ ``tools/get_stockfish.py`` đặt binary.
    4. ``stockfish`` trên ``$PATH``.
    5. ``chess_ai.config.STOCKFISH_PATH`` — dùng chung với đường chạy AIBOX.
    6. Các vị trí cài đặt hệ thống thông thường.
    """
    if explicit:
