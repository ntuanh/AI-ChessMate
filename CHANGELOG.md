...
- **Digital board** (`digital_board/web/index.html`) — a self-contained page, no CDN and no build
...
- **`digital_board.engine`** — one persistent Stockfish process behind `hint()`, `evaluate()` and
...
- **`digital_board.game`** — game state, server-side legality, undo, auto-queen promotion, and the
...
- **`tests/test_digital_board.py`** — 63 checks across game state, score formatting, engine discovery, real
